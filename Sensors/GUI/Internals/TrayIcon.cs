...
namespace GUI.Internals
{
    internal class TrayIcon
    {
        ...
        private ISensorMetadata _metadata;

        public event EventHandler<ModeChangedEventArgs> ModeChanged;
        public event EventHandler<StatusChangedEventArgs> StatusChanged;
        public event EventHandler<ApplicationExitEventArgs> ApplicationExit;

        internal TrayIcon(ISensorMetadata metadata, SensorSettings settings, IEnumerable<string> modes, string currentMode)
        {
            _metadata = metadata;
            _settings = settings;
            _modes = modes.ToList();
        ...
        public void OnPhysicalStatusHasChanged(object sender, StatusChangedEventArgs e)
        {
            ...
            showBalloonTipPhysicalStatusHasChanged(e);
        }
        ...
        public void OnStatusChangesIn(object sender, StatusChangesInEventArgs e)
        {
            ...
            string text = e.TriggeredBy.ToString();
        ...
        private ToolStripItem createModeToolStrip(IEnumerable<string> modes, string currentMode)
        {
            ...
            foreach (ToolStripMenuItem item in modesItem.DropDownItems)
            {
                item.Click += (sender, e) =>
                {
                    ModeChanged?.Invoke(this, new ModeChangedEventArgs("", _metadata, ((ToolStripMenuItem)sender).Text));
                };
            }
        ...
        private ToolStripMenuItem createExitToolStrip()
        {
            ...
            exitItem.Click += (sender, args) =>
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    ApplicationExit?.Invoke(this, new ApplicationExitEventArgs("", _metadata));
                }
            };
        ...
        private void showBalloonTipPhysicalStatusHasChanged(StatusChangedEventArgs e)
        {
            string title = e.Status == SensorPowerStatus.On ?
                _translation.GetBallonTipTitleOn(e.Socket) : _translation.GetBallonTipTitleOff(e.Socket);
            string text = _translation.GetBalloonTipText(e.TimeStamp, e.TriggeredBy, e.Condition);
        ...
        private StatusChangedEventArgs buildStatusChangedEventArgs(object sender, SensorPowerStatus status)
        {
            ...
            return new StatusChangedEventArgs("", "", _metadata, socket, status);
        }
